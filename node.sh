...
CHAIN_ID=1 forge script script/deploy/01_DeployYieldOracle.s.sol -s 'configureDeployment()' --ffi
forge script script/deploy/01_DeployYieldOracle.s.sol --rpc-url $RPC_URL --private-key $private_key --broadcast --slow
...
forge script script/deploy/02_DeployInterestRateModule.s.sol --rpc-url $RPC_URL --private-key $private_key --broadcast --slow
echo "DEPLOYING WHITELIST"
forge script script/deploy/03_DeployWhitelist.s.sol --rpc-url $RPC_URL --private-key $private_key --broadcast --slow
...
jq --arg interest_rate "$interest_rate_addr" --arg whitelist "$whitelist_addr" --arg initial_admin "$initial_admin" '. + { "initialDefaultAdmin": $initial_admin, "interestRateModule": $interest_rate, whitelist: $whitelist }' deployment-config/04_DeployIonPool.json >temp.json && mv temp.json deployment-config/04_DeployIonPool.json

# Deploy IonPool!
echo "DEPLOYING ION POOL..."
forge script script/deploy/04_DeployIonPool.s.sol --rpc-url $RPC_URL --private-key $private_key --broadcast --slow

# Deploy Oracles
echo "DEPLOYING ORACLES..."
forge script script/deploy/05_DeployInitialReserveAndSpotOracles.s.sol --rpc-url $RPC_URL --private-key $private_key --broadcast --slow
...
# Deploy Handlers
forge script script/deploy/08_DeployInitialHandlers.s.sol --rpc-url $RPC_URL --private-key $private_key --broadcast --slow --tc DeployInitialHandlersScript

# Deploy Liquidation
echo "DEPLOYING LIQUIDATION..."
protocol_addr=$(jq .treasury "deployment-config/04_DeployIonPool.json" | xargs) # copies from IonPool configuration
