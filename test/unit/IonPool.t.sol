...
// import { WAD, RAY, RAD, RoundedMath } from "../../src/math/RoundedMath.sol";
...
//             (uint256 collateral1, uint256 normalizedDebt1) = ionPool.vaults(i, borrower1);
//             (uint256 collateral2, uint256 normalizedDebt2) = ionPool.vaults(i, borrower2);
...
//         int256 normalizedDebtWad = normalizedDebt * int256(WAD);

//         (uint256 borrowRateBefore,) = ionPool.getCurrentBorrowRate(stEthIndex);

//         vm.expectEmit(true, true, true, true);
//         emit ModifyPosition(stEthIndex, borrower1, borrower1, borrower1, collateralToDeposit, normalizedDebtWad);
//         _borrowHelper(stEthIndex, borrower1, normalizedDebtWad);
...
//         // rate accumulator is 1 at this point
//         int256 debt = 10;
//         int256 debtWad = debt * int256(WAD);

//         uint256 initialLiquidity = underlying.balanceOf(address(ionPool));

//         _borrowHelper(stEthIndex, borrower1, debtWad);

//         (uint256 borrowRateBefore,) = ionPool.getCurrentBorrowRate(stEthIndex);
//         uint256 borrower1DebtBefore = ionPool.rate(stEthIndex) * ionPool.normalizedDebt(stEthIndex, borrower1); //
// [RAD]
...
//         );
//         _repayHelper(stEthIndex, borrower1, -int256(originalDebtNormalized));

//         (uint256 borrowRateAfter,) = ionPool.getCurrentBorrowRate(stEthIndex);
//         uint256 borrower1DebtAfter = ionPool.rate(stEthIndex) * ionPool.normalizedDebt(stEthIndex, borrower1); //
// [RAD]
...
//     function test_updateGlobalDebtCeiling() external {
//         uint256 newGlobalDebtCeiling = 10_000e45;
...
//         );
//         vm.prank(NON_ADMIN);
//         ionPool.updateGlobalDebtCeiling(newGlobalDebtCeiling);

//         ionPool.updateGlobalDebtCeiling(newGlobalDebtCeiling);
//         assertEq(ionPool.globalDebtCeiling(), newGlobalDebtCeiling);
//     }
