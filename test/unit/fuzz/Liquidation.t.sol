...
contract LiquidationFuzzFixedConfigsFixedRate is LiquidationSharedSetup {
    ...
    function setUp() public override {
        ...
        // fuzz configs 
        minDepositAmt = 1; 
        maxDepositAmt = 10000e18 ;
        minBorrowAmt = 1;  
        maxBorrowAmt = 10000e18;  
     ...
     */
    function testFuzz_AllLiquidationCategories(
        uint256 depositAmt, 
        uint256 borrowAmt,
        uint256 exchangeRate
    ) public {
        ...
        // starting position must be safe
        // [wad] * [ray] <= [wad] * [ray] 
        vm.assume(stateArgs.normalizedDebt * stateArgs.rate <= (stateArgs.collateral * startingExchangeRate.scaleToRay(18)).rayMulDown(deploymentArgs.liquidationThreshold));

        // position needs to be unsafe after exchange rate change
        vm.assume(stateArgs.normalizedDebt * stateArgs.rate > (stateArgs.collateral * stateArgs.exchangeRate.scaleToRay(18)).rayMulDown(deploymentArgs.liquidationThreshold)); 
        ...
        if (results.category == 0) { // protocol 
        ...
        } else if (results.category == 1) { // dust 
            ...
            if (actualNormalizedDebt != 0) {
                ...
                uint256 healthRatio = getHealthRatio(actualCollateral, actualNormalizedDebt, stateArgs.rate, stateArgs.exchangeRate, deploymentArgs.liquidationThreshold); 
    ...
    function testFuzz_ProtocolLiquidations(
        uint256 depositAmt, 
        uint256 borrowAmt,
        uint256 exchangeRate
    ) public {
    ...
    // lq = 0.9 ray 
    // 
    function testFuzz_PartialLiquidations(
        uint256 depositAmt, 
        uint256 borrowAmt,
        uint256 exchangeRate
    ) public {
        ...
        // starting position must be safe 
        stateArgs.normalizedDebt = bound(borrowAmt, 1, (stateArgs.collateral * startingExchangeRate.scaleToRay(18)).rayMulDown(deploymentArgs.liquidationThreshold) / stateArgs.rate); // [wad] 
        // position must be unsafe after exchange rate change        
        stateArgs.exchangeRate = bound(exchangeRate, minExchangeRate, (stateArgs.normalizedDebt * stateArgs.rate).rayDivDown(deploymentArgs.liquidationThreshold) / stateArgs.collateral - 1); // [ray] 
    ...
    function testFuzz_DustLiquidations(
        uint256 depositAmt, 
        uint256 borrowAmt,
        uint256 exchangeRate
    ) public {
