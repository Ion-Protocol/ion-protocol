...
//             (v, r, s) = vm.sign(sendingUserPrivateKey, MessageHashUtils.toTypedDataHash(DOMAIN_SEPARATOR,
// structHash));
//             (uint8 vMalleable, bytes32 rMalleable, bytes32 sMalleable) = _calculateMalleableSignature(v, r, s);

//             // Openzeppelin ECDSA library already prevents the use of malleable signatures, even if nonce-based
// replay
//             // protection wasn't included
//             vm.expectRevert(abi.encodeWithSelector(ECDSA.ECDSAInvalidSignatureS.selector, sMalleable));
//             rewardToken.permit(sendingUser, spender, amountOfRewardTokens, deadline, vMalleable, rMalleable,
// sMalleable);
