...
# 1. Test the script against a forked network. 
# 2. Test the script using call. 
# 3. Run the script on the network. 
# 4. Validate the post-execution state of the network. 
...
run_forge_script() {
    ...
    if [[ $mode == "test" ]]; then 
        echo "Running: forge script $script_path --tc $target_contract --fork-url $rpc_url --sender $sender $ffi_flag"
        forge script $script_path --tc $target_contract  --fork-url $rpc_url --sender $sender $ffi_flag
    elif [[ $mode == "prod" ]]; then 
        echo "Running: forge script $script_path --tc $target_contract --rpc-url $rpc_url --sender $sender $ffi_flag"
        forge script "$script_path" --tc "$target_contract"  --rpc-url $rpc_url --sender "$sender"  "$ffi_flag"
