...
uint32 constant PERIODS = 365 * APY_PRECISION / LOOK_BACK;
uint32 constant ILK_COUNT = 3;
uint256 constant UPDATE_LOCK_LENGTH = 1 days;
...
contract ApyOracle is IApyOracle {
    ...
    address public immutable LIDO_EXCHANGE_RATE_CONTRACT;
    address public immutable STADER_EXHCHANGE_RATE_CONTRACT;
    address public immutable SWELL_EXCHANGE_RATE_CONTRACT;
    ...
    constructor(
        ...
        LIDO_EXCHANGE_RATE_CONTRACT = _lido;
        STADER_EXHCHANGE_RATE_CONTRACT = _stader;
        SWELL_EXCHANGE_RATE_CONTRACT = _swell;
    ...
    function _getExchangeRate(uint256 ilkIndex) internal view returns (uint32 exchangeRate) {
        ...
        if (ilkIndex == 0) {
            ILidoWstEth lido = ILidoWstEth(LIDO_EXCHANGE_RATE_CONTRACT);
            exchangeRate = (lido.stEthPerToken() / scale).toUint32();
        } else if (ilkIndex == 1) {
            IStaderOracle stader = IStaderOracle(STADER_EXHCHANGE_RATE_CONTRACT);
            ...
            ISwellEth swell = ISwellEth(SWELL_EXCHANGE_RATE_CONTRACT);
