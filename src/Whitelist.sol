...
contract Whitelist is Ownable2Step {
    ...
    constructor(
        bytes32[] memory _borrowersRoots, 
        bytes32  _lendersRoot
    ) Ownable(msg.sender) {
    ...
    function updateBorrowersRoot(uint8 ilkIndex, bytes32 _borrowersRoot)
        external
        onlyOwner
    {
    ...
    function updateLendersRoot(bytes32 _lendersRoot)
        external
        onlyOwner
    {
        lendersRoot = _lendersRoot; 
     ...
     */ 
    function isWhitelistedBorrower(uint8 ilkIndex, address addr, bytes32[] calldata proof) external view returns (bool) {
